...
- You should be in a `lab-7-work` worktree (created in Step 0 of the participant guide). Confirm with `git worktree list` — your current directory should be the `lab-7-work` entry, not the main clone.
- **If you are editing from the main repo checkout (not a worktree), stop and set one up first:** `git worktree add ../lab-7-work lab-7-start`, then `cd ../lab-7-work` and relaunch Claude Code there.
